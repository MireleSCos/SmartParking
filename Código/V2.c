...
String samuel = " 49 ce 3b 59";
String selena = " 26 21 b8 f8";
...
// --- + --- + --- Verificando a Entrada dos veiculos  --- + --- + ---- //
    ...
    if (conteudo == autorizado ){
        ...
        lcd.print("Bem vinda,  ");   //Envia o texto entre aspas para o LCD
        lcd.setCursor(0,1);
        lcd.print(" Senhorita Mirele !");
        AbrirCancelaEnt(); 
        delay(200);
        ExibindoLCD();

    }else if ( conteudo == samuel) {
        Serial.println("\n------------ Veiculo autorizado------"); 
        lcd.clear();              //Limpa a tela
        lcd.setCursor(0,0);     //Posiciona o cursor na coluna 3, linha 0;
        lcd.print("Bem vindo,  ");   //Envia o texto entre aspas para o LCD
        lcd.setCursor(0,1);
        lcd.print(" Senhor Samuel !");
        AbrirCancelaEnt(); 
        delay(200);
        ExibindoLCD();
    }else if ( conteudo == selena) {
        Serial.println("\n------------ Veiculo autorizado------"); 
        lcd.clear();              //Limpa a tela
        lcd.setCursor(0,0);     //Posiciona o cursor na coluna 3, linha 0;
        lcd.print("Bem vinda,  ");   //Envia o texto entre aspas para o LCD
        lcd.setCursor(0,1);
        lcd.print(" Senhorita Selena !");
        AbrirCancelaEnt(); 
        delay(200);
        ExibindoLCD();
...
void VerificarVagas(){
    ...
    for (int i = 0; i < QTD_VAGAS; i++)     // Lendo os valores dos sensores
    {
        ...
        if(situ_Vaga <= 3){                // Menor valor = menor luz = vaga ocupada = LED apagado
